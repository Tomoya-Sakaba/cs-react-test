...
**このシステムでは外部キー制約を設定しません。**

**理由**:
1. **テーブル変更の柔軟性**: 将来的なスキーマ変更時に外部キー制約がボトルネックにならない
2. **運用の簡便性**: データ移行やメンテナンス時のロック問題を回避
3. **パフォーマンス**: INSERT/UPDATE時の外部キーチェックのオーバーヘッドを削減

**データ整合性の管理方法**:
- アプリケーション層でバリデーションを実施
- マスタデータ（会社、種別）の削除は原則禁止（論理削除）
- 参照整合性エラーは保存前にフロントエンドでチェック
