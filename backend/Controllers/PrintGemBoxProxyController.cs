...
namespace backend.Controllers
{
    ...
    public class PrintGemBoxProxyController : ApiController
    {
        ...
        /// <summary>クエリ <c>report</c> でペイロードを組み立て、backend-print の Excel 出力へ POST 転送する。</summary>
        [HttpGet]
        [Route("excel")]
        public async Task<HttpResponseMessage> GetGemBoxExcel(string report, int? reportNo = null)
        {
            if (string.IsNullOrWhiteSpace(report))
            {
                Log.Error("PrintGemBox GetGemBoxExcel: report が空です。");
                return CreateUniformPrintErrorResponse();
            }
            ...
            {
                Log.Error(
                    $"PrintGemBox BuildGemBoxPdfRequest(excel): ArgumentException（report={report}, reportNo={reportNo}）。{ex.Message}",
                    ex);
                return CreateUniformPrintErrorResponse();
            }
            catch (InvalidOperationException ex)
            {
                Log.Error(
                    $"PrintGemBox BuildGemBoxPdfRequest(excel): InvalidOperationException（report={report}, reportNo={reportNo}）。{ex.Message}",
                    ex);
                return CreateUniformPrintErrorResponse();
            }
            ...
            {
                Log.Error(
                    $"PrintGemBox: Excel 用ペイロードが null（report={reportFromRequest}, reportNo={reportNo}）。");
                return CreateUniformPrintErrorResponse();
            }

            try
            {
                return await _printProxy.ForwardGemBoxExcelPostAsync(gemBoxPrintRequest, Request).ConfigureAwait(false);
            }
            catch (InvalidOperationException ex)
            {
